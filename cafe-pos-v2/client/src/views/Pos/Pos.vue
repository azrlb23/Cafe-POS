...
const lastOrder = computed(() => {
    ...
    return todayOrders.value.find(o => o.id === lastOrderId.value);
});
...
const handlePaymentSubmit = async (paymentData) => {
    ...
    try {
        ...
        // setTimeout(() => PrinterService.printCashier(newOrder), 1200);
        // setTimeout(() => PrinterService.printCustomer(newOrder), 2400);
...
const handleManualPrint = (order) => {
    ...
    console.log("Printing disabled", order);
};
</script>
...
<template>
            ...
            <div class="flex-1 overflow-y-auto p-6 lg:p-8 pb-24 lg:pb-8 bg-[#F8F9FD] flex flex-col gap-8 no-scrollbar">
                ...
                <div class="relative w-full max-w-md">
                    ...
                    <input 
                        ...
                        class="w-full bg-white border border-slate-200 rounded-full pl-11 pr-4 py-3 text-sm font-bold focus:bg-slate-50 focus:ring-4 focus:ring-amber-600/10 transition-all shadow-sm"
                    >
                </div>
        ...
        <PaymentModal :show="showPaymentModal" :total="cartTotal" :processing="isProcessingOrder" @close="showPaymentModal = false" @submit="handlePaymentSubmit" />
