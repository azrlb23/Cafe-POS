            ...
            <div class="p-8">
                ...
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        <button v-for="n in [1,2,3,4,5,6,7,8,9]" :key="n" type="button" @click="appendPin(n.toString())" class="h-16 rounded-2xl bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-900 text-xl font-bold transition-all active:scale-90 cursor-pointer">
                            {{ n }}
                        </button>
                        <button type="button" @click="clearPin" class="h-16 rounded-2xl bg-red-50 hover:bg-red-100 border border-red-200 text-red-500 font-bold transition-all active:scale-90 uppercase text-xs tracking-widest cursor-pointer">Clear</button>
                        <button type="button" @click="appendPin('0')" class="h-16 rounded-2xl bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-900 text-xl font-bold transition-all active:scale-90 cursor-pointer">0</button>
                        <button type="submit" :disabled="form.pin.length < 6 || processing" class="h-16 rounded-2xl bg-amber-600 hover:bg-amber-700 text-white font-black transition-all active:scale-90 disabled:opacity-50 shadow-md cursor-pointer">
                            {{ processing ? '...' : 'OK' }}
                        </button>
                    </div>
                </form>
