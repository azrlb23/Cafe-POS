...
const filteredOrders = computed(() => {
    ...
    return props.todayOrders.filter(o => 
        ...
        (o.cafeTable?.name && o.cafeTable.name.toLowerCase().includes(q)) ||
                    ...
                    <div v-if="lastOrder" class="space-y-6">
                        ...
                        <div class="bg-white border border-stone-200 rounded-[2.2rem] p-6 shadow-md relative overflow-hidden">
                            ...
                            <div class="text-center pb-4 border-b border-dashed border-stone-200">
                                ...
                                <p class="text-base font-black text-stone-850 mt-1 font-mono tracking-tight font-serif">{{ lastOrder.orderNumber }}</p>
                                <div class="flex items-center justify-center gap-2 mt-2 flex-wrap">
                                    <span :class="lastOrder.orderType === 'dine_in' ? 'bg-[#3F6253]/10 text-[#3F6253] border border-[#3F6253]/20' : 'bg-[#B45309]/10 text-[#B45309] border border-[#B45309]/20'"
                                          class="px-2.5 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest border">
                                        {{ lastOrder.orderType === 'dine_in' ? 'Dine In' : 'Takeaway' }}
                                    </span>
                                    <span v-if="lastOrder.cafeTable" class="bg-stone-100 text-stone-600 border border-stone-200 px-2.5 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest">
                                        Meja {{ lastOrder.cafeTable.number }}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="py-4 border-b border-dashed border-stone-200 space-y-2">
                                <div v-for="item in lastOrder.orderItems" :key="item.id" class="flex justify-between text-xs font-bold text-stone-700">
                                    <span class="truncate pr-4 font-serif">{{ item.menuName }} <span class="text-amber-700 font-bold font-jakarta">x{{ item.quantity }}</span></span>
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="grid grid-cols-3 gap-3">
                                <router-link 
                                    :to="{ name: 'PrintReceipt', params: { id: lastOrder.id }, query: { type: 'customer' } }"
                                    target="_blank"
                                ...
                                <router-link 
                                    :to="{ name: 'PrintReceipt', params: { id: lastOrder.id }, query: { type: 'cashier' } }"
                                    target="_blank"
                                ...
                                <router-link 
                                    :to="{ name: 'PrintReceipt', params: { id: lastOrder.id }, query: { type: 'kitchen' } }"
                                    target="_blank"
                        ...
                        <div v-for="order in paginatedOrders" :key="order.id" class="bg-white border border-stone-200 rounded-2xl p-4 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                            <div class="flex-grow min-w-0">
                                <div class="flex items-center gap-2 mb-1.5 flex-wrap">
                                    <span class="text-xs font-black text-stone-855 font-jakarta tracking-tight font-serif">{{ order.orderNumber }}</span>
                                    <span :class="order.orderType === 'dine_in' ? 'bg-[#3F6253]/10 text-[#3F6253] border border-[#3F6253]/20' : 'bg-[#B45309]/10 text-[#B45309] border border-[#B45309]/20'"
                                          class="px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest border shrink-0">
                                        {{ order.orderType === 'dine_in' ? 'Dine In' : 'Takeaway' }}
                                    </span>
                                    <span v-if="order.cafeTable" class="bg-stone-100 text-stone-600 border border-stone-200 px-2.5 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest shrink-0">
                                        Meja {{ order.cafeTable.number }}
                                    </span>
                                </div>
                                <p class="text-[9px] font-black text-stone-400 uppercase tracking-widest flex items-center gap-1.5 mt-1">
                                    <span><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" class="inline mb-0.5 mr-0.5"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 15 15"/></svg> {{ formatTime(order.createdAt) }}</span>
                                    <span>•</span>
                                    <span class="text-amber-700 font-jakarta">Rp {{ Number(order.total).toLocaleString('id-ID') }}</span>
                                    <span>•</span>
                                    <span class="text-stone-500 normal-case font-bold">via {{ order.paymentMethod.toUpperCase() }}</span>
                                </p>
                            </div>
                            ...
                            <div class="flex items-center gap-2 shrink-0 flex-wrap">
                                <router-link 
                                    :to="{ name: 'PrintReceipt', params: { id: order.id }, query: { type: 'customer' } }"
                                    target="_blank"
                                ...
                                <router-link 
                                    :to="{ name: 'PrintReceipt', params: { id: order.id }, query: { type: 'cashier' } }"
                                    target="_blank"
                                ...
                                <router-link 
                                    :to="{ name: 'PrintReceipt', params: { id: order.id }, query: { type: 'kitchen' } }"
                                    target="_blank"
