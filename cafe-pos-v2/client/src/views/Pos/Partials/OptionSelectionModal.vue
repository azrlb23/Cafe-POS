...
const initOptions = () => {
    ...
    const groups = props.menu.menuOptionGroups || props.menu.menuOptionGroups || [];
...
const totalPrice = computed(() => {
    ...
    const groups = props.menu.menuOptionGroups || props.menu.menuOptionGroups || [];
    groups.forEach(group => {
        const items = group.menuOptionItems || [];
        items.forEach(item => {
            if (isSelected(group.id, item.id)) {
                extra += parseFloat(item.priceModifier || 0);
            }
        });
    });
    return parseFloat(props.menu.basePrice || 0) + extra;
});
