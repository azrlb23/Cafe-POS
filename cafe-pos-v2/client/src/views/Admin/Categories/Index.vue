...
const categories = computed(() => {
    let list = adminStore.categories;
...
const fetchData = async (force = false) => {
    if (!adminStore.isCategoriesLoaded) {
        isLoading.value = true;
    }
    try {
        await adminStore.fetchCategories(force);
    } catch (error) {
...
const deleteCategory = async (id) => {
    if (!confirm('Yakin ingin menghapus kategori ini? Menu yang berelasi mungkin akan terpengaruh.')) return;
    try {
        await api.delete(`/admin/categories/${id}`);
        flashMessage.value = 'Kategori berhasil dihapus';
                            ...
                            <tr v-for="category in categories" :key="category.id" class="group bg-white hover:bg-slate-50 transition-all duration-500 shadow-sm hover:shadow-xl hover:shadow-slate-200/50 rounded-3xl hover:-translate-y-1">
                                ...
                                <td class="py-6 px-8 text-right last:rounded-r-[2rem]">
                                    <div class="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition-all transform translate-x-4 group-hover:translate-x-0">
                                        <router-link
                                            :to="{ name: 'AdminCategoriesEdit', params: { id: category.id } }"
                ...
                <div class="lg:hidden grid grid-cols-1 md:grid-cols-2 gap-6 -mx-4 sm:mx-0 px-4 sm:px-0">
                    ...
                    >
                        <div class="flex items-center justify-between mb-4">
                            <span class="text-xs font-black text-slate-300">#{{ String(category.id).padStart(2, '0') }}</span>
                            <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest bg-slate-50 border border-slate-100 px-2 py-0.5 rounded-lg">
                                /{{ category.slug }}
                            </span>
                        </div>
                        <div class="font-serif font-black text-slate-900 text-xl mb-6">{{ category.name }}</div>
                    ...
                    <div v-if="categories.length === 0" class="col-span-full py-16 text-center bg-white rounded-3xl border border-dashed border-slate-200 px-4">
                        <div class="text-slate-400 font-black uppercase text-[10px] tracking-widest">Belum ada kategori terdaftar</div>
                    </div>
                </div>
            </div>
