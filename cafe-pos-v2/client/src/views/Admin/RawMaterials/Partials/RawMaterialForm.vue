...
const form = ref({
    ...
    cost_per_unit: props.material?.costPerUnit || props.material?.cost_per_unit || 0,
    current_stock: props.material?.currentStock || props.material?.current_stock || 0,
    minimum_stock: props.material?.minimumStock || props.material?.minimum_stock || 0,
    par_level: props.material?.parLevel || props.material?.par_level || 0,
    default_supplier_id: props.material?.defaultSupplierId || props.material?.default_supplier_id || ''
});
...
const submit = async () => {
    ...
    try {
        const payload = {
            ...
            current_stock: form.value.current_stock,
            minimum_stock: form.value.minimum_stock,
            par_level: form.value.par_level,
            default_supplier_id: form.value.default_supplier_id
        };

        if (isEdit) {
            await api.put(`/admin/raw-materials/${props.material.id}`, payload);
        } else {
            await api.post('/admin/raw-materials', payload);
        }
        router.push({ name: 'AdminRawMaterials' });
    } catch (e) {
        console.error("Failed to save raw material", e);
    ...
    <form @submit.prevent="submit" class="pb-40">
        ...
        <div class="fixed bottom-10 left-1/2 -translate-x-1/2 w-[95%] max-w-5xl bg-slate-900 border border-amber-700/30 shadow-[0_30px_60px_rgba(0,0,0,0.4)] rounded-[2.5rem] p-5 flex justify-between items-center z-[100] backdrop-blur-xl bg-opacity-95">
            ...
            <div class="flex gap-4 w-full md:w-auto">
                <router-link :to="{ name: 'AdminRawMaterials' }" class="flex-1 md:flex-none text-center px-10 py-4 rounded-2xl text-slate-400 hover:text-white hover:bg-white/5 transition-all font-black text-xs uppercase tracking-[0.2em] border border-slate-800 cursor-pointer">Batal</router-link>
                <button type="submit" :disabled="isProcessing" class="flex-1 md:flex-none bg-gradient-to-r from-amber-700 to-amber-800 hover:from-amber-800 hover:to-amber-900 text-white px-14 py-4 rounded-2xl font-black transition-all duration-300 shadow-xl shadow-amber-700/20 text-xs uppercase tracking-[0.2em] disabled:opacity-50 hover:scale-105 active:scale-95 cursor-pointer">
                    {{ isProcessing ? 'Memproses...' : 'Simpan Data Bahan' }}
                </button>
            </div>
        </div>
    </form>
