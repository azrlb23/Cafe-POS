...
const submit = async () => {
    ...
    try {
        ...
        if (isEdit) {
            await api.put(`/admin/cashiers/${props.cashier.id}`, payload);
        } else {
            await api.post('/admin/cashiers', payload);
        }
        router.push({ name: 'AdminCashiers' });
    } catch (e) {
        console.error("Failed to save cashier", e);
        if (e.response?.status === 422 || e.response?.status === 400) {
            formErrors.value = e.response.data.errors || {};
            alert(e.response.data.message || 'Validasi gagal.');
        } else {
    ...
    <form @submit.prevent="submit" class="pb-40">
            ...
            <section class="grid grid-cols-1 xl:grid-cols-3 gap-12 lg:gap-20">
                ...
                <div class="xl:col-span-2 space-y-6">
                    <div>
                        ...
                        <div v-if="formErrors.name" class="text-red-500 text-xs mt-2 ml-1">{{ formErrors.name }}</div>
                    </div>
                    ...
                    <div>
                        ...
                        <div v-if="formErrors.email" class="text-red-500 text-xs mt-2 ml-1">{{ formErrors.email }}</div>
                    </div>
                </div>
            </section>
                ...
                <div class="xl:col-span-2 space-y-6">
                    <div>
                        <div class="flex justify-between items-center mb-3">
                            ...
                            <button type="button" @click="generateRandomPin" class="text-[10px] font-black text-amber-700 hover:text-amber-800 uppercase tracking-widest transition-colors cursor-pointer">Generate PIN</button>
                        </div>
                        ...
                        <div v-if="formErrors.pin" class="text-red-500 text-xs mt-2 ml-1">{{ formErrors.pin }}</div>
                    </div>
                    ...
                    <div>
                        ...
                        <div v-if="formErrors.password" class="text-red-500 text-xs mt-2 ml-1">{{ formErrors.password }}</div>
                    </div>
                </div>
            </section>
        </div>
        ...
        <div class="fixed bottom-10 left-1/2 -translate-x-1/2 w-[95%] max-w-5xl bg-[#1C1E23] border border-amber-700/30 shadow-[0_30px_60px_rgba(0,0,0,0.4)] rounded-[2.5rem] p-5 flex justify-between items-center z-[100] backdrop-blur-xl bg-opacity-95">
            ...
            <div class="flex gap-4 w-full md:w-auto">
                <router-link :to="{ name: 'AdminCashiers' }" class="flex-1 md:flex-none text-center px-10 py-4 rounded-2xl text-slate-400 hover:text-white hover:bg-white/5 transition-all font-black text-xs uppercase tracking-[0.2em] border border-slate-800 cursor-pointer">Batal</router-link>
                <button type="submit" :disabled="isProcessing" class="flex-1 md:flex-none bg-gradient-to-r from-amber-700 to-amber-800 hover:from-amber-800 hover:to-amber-900 text-white px-14 py-4 rounded-2xl font-black transition-all duration-300 shadow-xl shadow-amber-700/20 text-xs uppercase tracking-[0.2em] disabled:opacity-50 hover:scale-105 active:scale-95 cursor-pointer">
                    {{ isProcessing ? 'Memproses...' : 'Simpan Kasir' }}
                </button>
            </div>
        </div>
    </form>
