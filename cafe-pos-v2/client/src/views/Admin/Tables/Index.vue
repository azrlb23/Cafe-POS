...
const openTableDetails = (table) => {
    // Check if table has orders and get the first one (active order)
    const activeOrder = table.orders && table.orders.length > 0 ? table.orders[0] : null;
    
    if (activeOrder) {
        selectedTable.value = { ...table, active_order: activeOrder };
...
<template>
            ...
            <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8 delay-100">
                ...
                >
                    ...
                    <div class="flex justify-between items-center mb-4">
                        ...
                        >
                            {{ !table.orders || table.orders.length === 0 ? 'Tersedia' : 'Terisi' }}
                        </span>
                    </div>
        ...
        >
            <div v-if="isModalOpen && selectedTable?.active_order" class="fixed inset-0 z-50 flex items-center justify-center p-4">
                <!-- Backdrop -->
                ...
                <div class="relative bg-white w-full max-w-lg rounded-[3rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
                    <div class="p-10">
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                                        #{{ selectedTable.active_order.orderNumber || selectedTable.active_order.order_number }}
                                    </span>
                                </div>
                            </div>
                            <button 
                                @click="isModalOpen = false"
                                class="w-12 h-12 rounded-2xl bg-slate-50 text-slate-400 hover:text-slate-900 transition-colors flex items-center justify-center cursor-pointer"
                            >
                        ...
                        <div class="space-y-4 mb-10 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                            ...
                            <div v-for="item in (selectedTable.active_order.orderItems || selectedTable.active_order.order_items)" :key="item.id" class="flex justify-between items-center p-4 bg-white border border-slate-100 rounded-2xl shadow-sm">
