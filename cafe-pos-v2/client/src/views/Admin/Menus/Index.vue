...
const confirmDeletion = async (menu) => {
    if (confirm(`Yakin ingin menghapus menu "${menu.name}"? Semua resep dan opsi akan ikut terhapus selamanya.`)) {
        try {
            ...
            console.error(e);
            alert(e.response?.data?.message || 'Gagal menghapus menu');
        }
    }
};
</script>
...
<template>
            ...
            <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
                <div>
                    <h2 class="text-2xl sm:text-3xl lg:text-4xl font-serif font-black text-slate-800 tracking-tight leading-tight">
                        Katalog <span class="text-amber-700 italic">Menu</span>
                    </h2>
                    ...
                    <div class="flex items-center gap-3 mt-2 text-[10px] sm:text-xs font-bold text-slate-500">
                        <span>{{ menus.length }} Menu Terdaftar</span>
                ...
                <router-link
                    :to="{ name: 'AdminMenusCreate' }"
                    class="inline-flex items-center justify-center gap-2 bg-amber-700 hover:bg-amber-800 text-white px-5 py-2.5 rounded-xl font-black transition-all text-[10px] uppercase tracking-wider active:scale-95 text-center cursor-pointer"
                >
            ...
            <div class="flex flex-col lg:flex-row gap-6 mb-10 items-stretch lg:items-end delay-100">
                <div class="flex-1 w-full">
                    <label class="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 ml-1">Pencarian Menu</label>
                    <div class="relative group">
                        <span class="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-amber-700 transition-colors">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                        </span>
                        <input v-model="search" type="text" placeholder="Cari nama kopi, snack, atau minuman..." class="w-full bg-white border border-slate-200 rounded-2xl pl-14 pr-6 py-4 focus:border-amber-700 focus:ring-4 focus:ring-amber-700/10 transition-all text-sm font-bold shadow-sm">
                    </div>
                </div>
                ...
                <button @click="resetFilters" class="w-full lg:w-auto px-8 py-4 bg-slate-100 hover:bg-red-50 text-slate-500 hover:text-red-600 rounded-2xl transition-all text-[10px] font-black uppercase tracking-widest border border-slate-200 active:scale-95 cursor-pointer text-center">
                    Reset
                </button>
            </div>

            <div v-if="isLoading" class="py-32 text-center bg-white rounded-[3rem] border border-dashed border-slate-200 flex justify-center items-center">
