...
// Initialize form
const form = ref({
    category_id: props.menu?.categoryId || props.menu?.category_id || '',
    name: props.menu?.name || '',
    description: props.menu?.description || '',
    base_price: props.menu?.basePrice || props.menu?.base_price || 0,
    is_active: props.menu?.isActive ?? props.menu?.is_active ?? true,
    image: null,
    
    // Deeply nested arrays
    recipes: props.menu?.recipes?.map(r => ({
        id: r.id,
        raw_material_id: r.rawMaterialId || r.raw_material_id,
        quantity: r.quantity
    })) || [],
    
    option_groups: props.menu?.menuOptionGroups?.map(g => ({
        id: g.id,
        name: g.name,
        min_select: g.minSelect || g.min_select,
        max_select: g.maxSelect || g.max_select,
        items: g.menuOptionItems?.map(i => ({
            id: i.id,
            name: i.name,
            price_modifier: i.priceModifier || i.price_modifier,
            recipes: i.recipes?.map(ir => ({
                id: ir.id,
                raw_material_id: ir.rawMaterialId || ir.raw_material_id,
...
const imagePreview = ref<string | null>(
    (props.menu?.imagePath || props.menu?.image_path)
        ? '/storage/' + (props.menu.imagePath || props.menu.image_path)
        : null
);
...
const submitForm = async () => {
    ...
    try {
        // BUG-001 fix: Use FormData to support multipart/form-data file upload
        ...
        fd.append('recipes', JSON.stringify(form.value.recipes));
        fd.append('option_groups', JSON.stringify(form.value.option_groups));
        if (form.value.image) {
            fd.append('image', form.value.image);
        }
        
        if (isEdit) {
            await api.put(`/admin/menus/${props.menu.id}`, fd, {
                headers: { 'Content-Type': 'multipart/form-data' },
            });
        } else {
            await api.post('/admin/menus', fd, {
                headers: { 'Content-Type': 'multipart/form-data' },
            });
        }
        
        router.push({ name: 'AdminMenus' });
    } catch (e) {
        if (e.response && e.response.status === 422) {
            const errors = e.response.data.errors;
            if (errors) {
                // Map errors manually (e.g., errors.name[0])
...
const addBaseRecipe = () => {
    form.value.recipes.push({ raw_material_id: '', quantity: 0 });
};
...
const addOptionGroup = () => {
    form.value.option_groups.push({ name: '', min_select: 0, max_select: 1, items: [] });
};
...
const addOptionItem = (groupIndex) => {
    form.value.option_groups[groupIndex].items.push({ name: '', price_modifier: 0, recipes: [] });
};
const removeOptionItem = (groupIndex, itemIndex) => {
    if (confirm('Hapus pilihan ini?')) {
        form.value.option_groups[groupIndex].items.splice(itemIndex, 1);
    }
};

const addOptionItemRecipe = (groupIndex, itemIndex) => {
    form.value.option_groups[groupIndex].items[itemIndex].recipes.push({ raw_material_id: '', quantity: 0 });
};
const removeOptionItemRecipe = (groupIndex, itemIndex, recipeIndex) => {
    form.value.option_groups[groupIndex].items[itemIndex].recipes.splice(recipeIndex, 1);
};
</script>
...
<template>
    <form @submit.prevent="submitForm" class="pb-32 font-sans text-slate-900 max-w-[1400px] mx-auto">
            ...
            <section class="grid grid-cols-1 xl:grid-cols-3 gap-12 lg:gap-20">
                <div class="xl:col-span-1">
                    <div class="flex justify-between items-start xl:block">
                        ...
                        <button type="button" @click="addBaseRecipe" class="mt-6 bg-slate-900 text-white hover:bg-amber-500 px-6 py-3 rounded-xl transition-all flex items-center gap-3 text-[10px] font-black uppercase tracking-widest shadow-xl shadow-slate-900/10 cursor-pointer">
                        ...
                        <div v-for="(recipe, index) in form.recipes" :key="'br-'+index" class="group flex items-center gap-6 bg-white p-2 pl-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md hover:border-amber-200 transition-all">
                            ...
                            <div class="w-48 relative">
                                ...
                                <span class="absolute right-6 top-1/2 -translate-y-1/2 text-[10px] text-slate-400 font-black uppercase tracking-widest">
                                    {{ (raw_materials as any[]).find((r: any) => r.id === recipe.raw_material_id)?.unit || 'Unit' }}
                                </span>
                            </div>
                            <button type="button" @click="removeBaseRecipe(index)" class="w-14 h-14 flex items-center justify-center text-slate-300 hover:text-red-500 transition-colors cursor-pointer">
            ...
            <section class="grid grid-cols-1 xl:grid-cols-3 gap-12 lg:gap-20">
                <div class="xl:col-span-1">
                    <div class="flex justify-between items-start xl:block">
                        ...
                        <button type="button" @click="addOptionGroup" class="mt-6 bg-amber-500 text-white hover:bg-amber-600 px-8 py-3 rounded-2xl transition-all shadow-xl shadow-amber-500/20 flex items-center gap-3 font-black text-[10px] uppercase tracking-[0.2em] cursor-pointer">
                            ...
                            <div class="bg-white rounded-[3rem] border border-slate-200 shadow-sm overflow-hidden">
                                ...
                                <div class="bg-slate-50 px-10 py-8 border-b border-slate-200 flex flex-col md:flex-row gap-8 items-start md:items-center">
                                    ...
                                    <div class="flex gap-4 w-full md:w-auto">
                                        ...
                                        <button type="button" @click="removeOptionGroup(gIndex)" class="self-end mb-1 w-12 h-12 flex items-center justify-center bg-white border border-red-100 text-red-400 hover:bg-red-500 hover:text-white rounded-xl transition-all shadow-sm cursor-pointer">
                                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div class="p-10 space-y-8">
                                    <div class="flex justify-between items-center px-2">
                                        ...
                                        <button type="button" @click="addOptionItem(gIndex)" class="text-[10px] font-black uppercase tracking-widest text-amber-600 hover:text-amber-700 flex items-center gap-2 cursor-pointer">
                                            ...
                                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                                                ...
                                                <div class="bg-white border border-slate-200 rounded-2xl p-5 relative">
                                                    <div class="flex justify-between items-center mb-4">
                                                        ...
                                                        <button type="button" @click="addOptionItemRecipe(gIndex, iIndex)" class="text-[9px] font-black text-amber-600 cursor-pointer">+ Bahan</button>
                                                    </div>
                                                    ...
                                                    <div class="space-y-2">
                                                        <div v-for="(iRecipe, irIndex) in item.recipes" :key="'ir-'+gIndex+'-'+iIndex+'-'+irIndex" class="flex items-center gap-2">
                                                            ...
                                                            <button type="button" @click="removeOptionItemRecipe(gIndex, iIndex, irIndex)" class="text-slate-300 hover:text-red-500 cursor-pointer">
                                                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                                                            </button>
                                                        </div>
                                                    </div>

                                                    <button type="button" @click="removeOptionItem(gIndex, iIndex)" class="absolute -top-3 -right-3 w-8 h-8 flex items-center justify-center bg-white border border-red-50 text-red-400 hover:bg-red-500 hover:text-white rounded-full shadow-md opacity-0 group-hover/item:opacity-100 transition-all cursor-pointer">
                                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                                                    </button>
                                                </div>
                                            </div>
        ...
        <div class="fixed bottom-10 left-1/2 -translate-x-1/2 w-[95%] max-w-5xl bg-slate-900 border border-amber-500/30 shadow-[0_30px_60px_rgba(0,0,0,0.4)] rounded-[2rem] p-5 flex justify-between items-center z-[100] backdrop-blur-xl bg-opacity-95">
            ...
            <div class="flex gap-4 w-full md:w-auto">
                <router-link :to="{ name: 'AdminMenus' }" class="flex-1 md:flex-none text-center px-10 py-4 rounded-2xl text-slate-400 hover:text-white hover:bg-white/5 transition-all font-black text-xs uppercase tracking-[0.2em] border border-slate-800 cursor-pointer">Batal</router-link>
                <button type="submit" :disabled="isProcessing" class="flex-1 md:flex-none bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white px-14 py-4 rounded-2xl font-black transition-all duration-300 shadow-xl shadow-amber-500/20 text-xs uppercase tracking-[0.2em] disabled:opacity-50 hover:scale-105 active:scale-95 cursor-pointer">
                    {{ isProcessing ? 'Memproses...' : 'Konfirmasi & Simpan' }}
                </button>
            </div>
        </div>
