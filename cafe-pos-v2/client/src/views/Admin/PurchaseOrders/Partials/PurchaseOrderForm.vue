...
// Fallback if DatePicker is not fully Vue 3 compatible, use native input date
...
watch(() => props.autofill, (newVal) => {
    if (newVal) {
        form.value.supplier_id = newVal.supplier_id || '';
        form.value.items = newVal.items?.map(i => ({
            raw_material_id: i.raw_material_id,
            quantity: i.quantity,
            unit_cost: i.unit_cost,
            name: i.name,
...
watch(() => props.nextOrderNumber, (newVal) => {
    if (newVal && !form.value.order_number) {
        form.value.order_number = newVal;
    }
});
...
const updateItemInfo = (index: any) => {
    const material = (props.rawMaterials as any[] || []).find(m => m.id === form.value.items[index].raw_material_id);
    if (material) {
        form.value.items[index].unit_cost = material.costPerUnit || material.cost_per_unit || 0;
...
const submit = async () => {
    ...
    try {
        ...
        router.push({ name: 'AdminPurchaseOrders' });
    } catch (e) {
...
const calculateTotal = () => {
    return form.value.items.reduce((acc, item) => acc + (Number(item.quantity) * Number(item.unit_cost)), 0);
};
</script>
    ...
    <form @submit.prevent="submit" class="pb-40">
                ...
                <div class="xl:col-span-2 space-y-12">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                        ...
                        <div class="col-span-2 md:col-span-1">
                            ...
                            <!-- Menggunakan native date karena kompatibilitas komponen khusus mungkin terganggu -->
                            <input v-model="form.purchase_date" type="date" class="w-full bg-white border border-slate-200 rounded-2xl px-8 py-4 focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 transition-all font-black text-lg shadow-sm" required>
                        </div>
                        ...
                        <div class="col-span-2">
                            ...
                            <select v-model="form.supplier_id" class="w-full bg-white border border-slate-200 rounded-[2rem] px-8 py-5 focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 transition-all font-bold text-xl shadow-sm" required>
                                ...
                                <option v-for="supplier in (suppliers as any[])" :key="supplier.id" :value="supplier.id">
                                    {{ supplier.name }}
                                </option>
                            </select>
                        </div>
            ...
            <section class="grid grid-cols-1 xl:grid-cols-3 gap-12 lg:gap-20">
                <div class="xl:col-span-1">
                    <div class="flex justify-between items-start xl:block">
                        ...
                        <button type="button" @click="addItem" class="mt-6 bg-slate-900 text-white hover:bg-amber-600 px-8 py-3 rounded-xl transition-all flex items-center gap-3 text-[10px] font-black uppercase tracking-widest shadow-xl shadow-slate-900/10 cursor-pointer">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-12 gap-8 items-end">
                                <div class="md:col-span-5 space-y-2">
                                    ...
                                    <select v-model="item.raw_material_id" @change="updateItemInfo(index)" class="w-full bg-slate-50 border-none rounded-2xl px-6 py-4 font-bold text-slate-900 focus:ring-2 focus:ring-amber-500" required>
                                        ...
                                        <option v-for="m in (rawMaterials as any[])" :key="m.id" :value="m.id">{{ m.name }}</option>
                                    </select>
                                </div>
                                <div class="md:col-span-3 space-y-2">
                                    ...
                                    <div class="relative">
                                        ...
                                        <span class="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-black text-slate-300 uppercase tracking-widest">
                                            {{ ((rawMaterials as any[]) || []).find(m => m.id === item.raw_material_id)?.unit || '' }}
                                        </span>
                                    </div>
                                </div>
                                <div class="md:col-span-3 space-y-2">
                                ...
                                <div class="md:col-span-1 flex justify-center pb-2">
                                    <button type="button" @click="removeItem(index)" class="text-slate-300 hover:text-red-500 transition-colors cursor-pointer">
        ...
        <div class="fixed bottom-10 left-1/2 -translate-x-1/2 w-[95%] max-w-5xl bg-slate-900 border border-amber-500/30 shadow-[0_30px_60px_rgba(0,0,0,0.4)] rounded-[2.5rem] p-5 flex justify-between items-center z-[100] backdrop-blur-xl bg-opacity-95">
            ...
            <div class="flex gap-4 w-full md:w-auto">
                <router-link :to="{ name: 'AdminPurchaseOrders' }" class="flex-1 md:flex-none text-center px-10 py-4 rounded-2xl text-slate-400 hover:text-white hover:bg-white/5 transition-all font-black text-xs uppercase tracking-[0.2em] border border-slate-800 cursor-pointer">Batal</router-link>
                <button type="submit" :disabled="isProcessing" class="flex-1 md:flex-none bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white px-14 py-4 rounded-2xl font-black transition-all duration-300 shadow-xl shadow-amber-500/20 text-xs uppercase tracking-[0.2em] disabled:opacity-50 hover:scale-105 active:scale-95 cursor-pointer">
                    {{ isProcessing ? 'Memproses...' : 'Simpan Stok Masuk' }}
                </button>
            </div>
        </div>
    </form>
