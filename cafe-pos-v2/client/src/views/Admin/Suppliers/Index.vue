...
const suppliers = computed(() => adminStore.suppliers);
const isLoading = ref(true);
const flashMessage = ref('');

const fetchData = async (force = false) => {
    if (!adminStore.isSuppliersLoaded) {
        isLoading.value = true;
    }
    try {
        await adminStore.fetchSuppliers(force);
    } catch (e) {
...
<template>
            ...
            <div v-else class="delay-200">
                ...
                <div class="lg:hidden grid grid-cols-1 md:grid-cols-2 gap-6 -mx-4 sm:mx-0 px-4 sm:px-0">
                    ...
                    >
                        <div>
                            <div class="font-serif font-black text-slate-900 text-xl mb-1">{{ supplier.name }}</div>
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4">{{ supplier.address || '-' }}</p>
                            
                            <div class="space-y-3 my-6">
                                <div class="flex items-center justify-between text-xs">
                                    <span class="font-medium text-slate-400">Kontak PIC:</span>
                                    <span class="font-bold text-slate-700">{{ supplier.contactPerson || supplier.contact_person || '-' }}</span>
                                </div>
                                <div class="flex items-center justify-between text-xs">
                                    <span class="font-medium text-slate-400">Telepon:</span>
                                    <span class="font-bold text-slate-700">{{ supplier.phone || '-' }}</span>
                                </div>
                                <div class="flex items-center justify-between text-xs">
                                    <span class="font-medium text-slate-400">Email:</span>
                                    <span class="font-medium text-slate-600">{{ supplier.email || '-' }}</span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div v-if="suppliers.length === 0" class="col-span-full py-16 text-center bg-white rounded-3xl border border-dashed border-slate-200 px-4">
                        <p class="text-slate-400 font-black uppercase text-[10px] tracking-widest leading-loose">Belum ada data supplier.</p>
                    </div>
                </div>
            </div>
