    ...
    <form @submit.prevent="submit" class="pb-40">
                ...
                <div class="xl:col-span-2 space-y-12">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                        <div class="col-span-2">
                            ...
                            <input v-model="form.name" type="text" placeholder="Misal: Biji Kopi Arabika" class="w-full bg-white border border-slate-200 rounded-[2rem] px-8 py-5 focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 transition-all font-bold text-xl shadow-sm" required>
                        </div>
                        
                        <div>
                            <label class="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 ml-1">Stok Saat Ini</label>
                            <div class="relative group">
                                <input v-model.number="form.current_stock" type="number" step="0.01" class="w-full bg-white border border-slate-200 rounded-2xl px-8 py-4 focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 transition-all font-black text-lg shadow-sm" required>
                        ...
                        <div>
                            ...
                            <input v-model="form.unit" type="text" placeholder="gr, ml, pcs" class="w-full bg-white border border-slate-200 rounded-2xl px-8 py-4 focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 transition-all font-bold text-lg shadow-sm" required>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section class="grid grid-cols-1 xl:grid-cols-3 gap-12 lg:gap-20">
                ...
                <div class="xl:col-span-2 space-y-12">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-10 bg-slate-50/50 p-10 rounded-[3rem] border border-slate-100">
                        <div class="space-y-2">
                            <label class="block text-[10px] font-black text-amber-600 uppercase tracking-[0.2em] ml-1">Minimum Stok (Threshold)</label>
                            <input v-model.number="form.minimum_stock" type="number" step="0.01" class="w-full bg-white border border-slate-200 rounded-2xl px-6 py-4 focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 transition-all font-bold shadow-sm">
                            <p class="text-[10px] text-slate-400 italic">Sistem akan memberi peringatan jika stok di bawah angka ini.</p>
                        </div>

                        <div class="space-y-2">
                            <label class="block text-[10px] font-black text-amber-600 uppercase tracking-[0.2em] ml-1">Par Level (Ideal)</label>
                            <input v-model.number="form.par_level" type="number" step="0.01" class="w-full bg-white border border-slate-200 rounded-2xl px-6 py-4 focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 transition-all font-bold shadow-sm">
                        ...
                        <div class="col-span-2 space-y-2 pt-4">
                            ...
                            <select v-model="form.default_supplier_id" class="w-full bg-white border border-slate-200 rounded-2xl px-6 py-4 focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 transition-all font-bold shadow-sm">
                ...
                <div class="xl:col-span-2">
                    <div class="max-w-md space-y-2">
                        ...
                        <div class="relative group">
                            ...
                            <input v-model.number="form.cost_per_unit" type="number" step="0.01" class="w-full bg-white border border-slate-200 rounded-2xl pl-14 pr-6 py-5 focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 transition-all font-black text-2xl shadow-sm" required>
                        </div>
                    </div>
                </div>
            </section>
        ...
        <div class="fixed bottom-10 left-1/2 -translate-x-1/2 w-[95%] max-w-5xl bg-slate-900 border border-amber-500/30 shadow-[0_30px_60px_rgba(0,0,0,0.4)] rounded-[2.5rem] p-5 flex justify-between items-center z-[100] backdrop-blur-xl bg-opacity-95">
            <div class="hidden md:flex items-center gap-5 pl-5">
                ...
                <div>
                    <span class="text-amber-500 font-serif text-xl font-bold tracking-wide">{{ isEdit ? 'Update Stok' : 'Daftarkan Bahan' }}</span>
