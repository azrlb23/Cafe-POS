            ...
            <div class="p-8">
                <div class="text-center mb-8">
                    <div class="w-16 h-16 bg-amber-100 rounded-2xl flex items-center justify-center mx-auto mb-4 border border-amber-200">
                        <svg class="text-amber-600" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
                    </div>
                    <h2 class="text-2xl font-serif font-black text-amber-600 uppercase tracking-widest">Buka Kasir</h2>
                ...
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div class="space-y-2">
                        <div class="flex justify-center gap-3">
                            <div v-for="i in 6" :key="i" 
                                :class="form.pin.length >= i ? 'bg-amber-500 scale-110 shadow-[0_0_15px_rgba(245,158,11,0.5)]' : 'bg-slate-200'"
                    ...
                    <div>
                        ...
                        <input v-model.number="form.opening_cash" type="number" min="0" step="1000" class="w-full bg-slate-50 border border-slate-200 rounded-2xl py-4 text-center text-2xl font-bold text-slate-900 focus:border-amber-400 focus:ring focus:ring-amber-200 focus:ring-opacity-50 transition-all outline-none" required>
                    </div>
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        ...
                        <button type="submit" :disabled="form.pin.length < 6 || form.processing" class="h-16 rounded-2xl bg-amber-500 hover:bg-amber-600 text-white font-black transition-all active:scale-90 disabled:opacity-50 shadow-md">
                            {{ form.processing ? '...' : 'OK' }}
                        </button>
                    </div>
                </form>
