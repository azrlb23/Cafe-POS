...
<template>
    ...
    <div class="min-h-screen bg-cafe-base text-cafe-main font-sans selection:bg-[#D97706] selection:text-white overflow-x-hidden transition-colors duration-300">
        ...
        <nav :class="['fixed w-full z-50 transition-all duration-300 border-b', isScrolled ? 'bg-cafe-surface/90 backdrop-blur-md border-cafe-border py-4 shadow-sm' : 'bg-transparent border-transparent py-6']">
            <div class="max-w-7xl mx-auto px-6 flex items-center justify-between">
                ...
                <a href="#" @click="closeMobileMenu" class="flex items-center gap-3">
                    <span class="font-bold text-2xl tracking-widest text-[#D97706] font-serif logo-anim">Denjavas</span>
                ...
                <div class="hidden md:flex gap-8 items-center">
                    <a href="#menu" :class="['text-[10px] font-black uppercase tracking-[0.3em] transition-all focus:outline-none', activeSection === 'menu' ? 'text-[#D97706]' : 'text-slate-500 hover:text-[#D97706]']">
                        Menu
                    </a>
                    <a href="#cara-pesan" :class="['text-[10px] font-black uppercase tracking-[0.3em] transition-all focus:outline-none', activeSection === 'cara-pesan' ? 'text-[#D97706]' : 'text-slate-500 hover:text-[#D97706]']">
                        Cara Pesan
                    </a>
                    <a href="#about" :class="['text-[10px] font-black uppercase tracking-[0.3em] transition-all focus:outline-none', activeSection === 'about' ? 'text-[#D97706]' : 'text-slate-500 hover:text-[#D97706]']">
                        Cerita
                    </a>
                    ...
                    <template v-if="$page.props.auth.user">
                        <Link
                            ...
                            class="text-[10px] font-black text-amber-600 hover:text-amber-700 transition-all uppercase tracking-[0.3em] flex items-center gap-2"
                        >
                            Ke Aplikasi
                ...
                <button @click="toggleMobileMenu" class="md:hidden p-2 text-cafe-main hover:text-[#D97706] focus:outline-none transition-colors">
            ...
            <div :class="['md:hidden absolute top-full left-0 w-full bg-cafe-surface/95 backdrop-blur-lg border-b border-cafe-border transition-all duration-300 overflow-hidden', isMobileMenuOpen ? 'max-h-64 opacity-100' : 'max-h-0 opacity-0']">
                <div class="flex flex-col px-6 py-4 gap-4">
                    <a href="#menu" @click="closeMobileMenu" :class="['text-sm font-bold uppercase tracking-widest', activeSection === 'menu' ? 'text-[#D97706]' : 'text-cafe-main']">Menu</a>
                    <a href="#cara-pesan" @click="closeMobileMenu" :class="['text-sm font-bold uppercase tracking-widest', activeSection === 'cara-pesan' ? 'text-[#D97706]' : 'text-cafe-main']">Cara Pesan</a>
                    <a href="#about" @click="closeMobileMenu" :class="['text-sm font-bold uppercase tracking-widest', activeSection === 'about' ? 'text-[#D97706]' : 'text-cafe-main']">Cerita</a>
                    <div class="w-full h-px bg-cafe-border my-2"></div>
                    <template v-if="$page.props.auth.user">
                        <Link :href="$page.props.auth.user.role === 'admin' ? route('dashboard') : route('pos')" class="text-sm font-bold text-[#D97706] uppercase tracking-widest flex items-center gap-2 w-fit">
                            Ke Aplikasi
                    ...
                    <template v-else>
                        <Link v-if="canLogin" :href="route('login')" class="text-sm font-bold text-[#D97706] uppercase tracking-widest flex items-center gap-2 w-fit">
                            Log in Staff
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4M10 17l5-5-5-5M15 12H3"/></svg>
                        </Link>
                    </template>
            ...
            <div class="relative z-10 w-full max-w-7xl mx-auto px-6 flex flex-col lg:flex-row items-center gap-16 mt-20">
                <div class="flex-1 text-center lg:text-left animate-fade-in-up">
                    <div class="mb-6 inline-block">
                        <span class="text-amber-600 font-black tracking-[0.4em] text-[10px] uppercase border border-amber-200 px-5 py-2 rounded-full bg-amber-50/50 backdrop-blur-sm shadow-sm">
                            Est. 2024
                        </span>
                    </div>
                    ...
                    <h1 class="text-6xl sm:text-7xl md:text-8xl font-serif font-black mb-8 leading-[0.9] tracking-tighter text-slate-900">
                        Authentic<br/>
                        <span class="text-transparent bg-clip-text bg-gradient-to-r from-amber-600 to-amber-400 italic font-light">Retro Café</span>
                    </h1>
                    
                    <p class="text-xl md:text-2xl text-amber-600 mb-6 font-serif italic font-black tracking-tight">
                    ...
                    <div class="flex flex-col sm:flex-row gap-5 items-center justify-center lg:justify-start w-full">
                        <a href="#menu" class="w-full sm:w-auto bg-amber-500 hover:bg-amber-600 text-white px-12 py-5 rounded-2xl font-black transition-all duration-300 shadow-xl shadow-amber-500/20 flex items-center justify-center gap-3 hover:-translate-y-1 active:scale-95 uppercase text-xs tracking-[0.2em]">
                            Eksplor Menu
        ...
        <section id="menu" class="py-32 relative bg-cafe-base transition-colors duration-300 scroll-mt-20">
            <div class="max-w-7xl mx-auto px-6">
                ...
                <div class="flex flex-col items-center text-center mb-20 reveal-element delay-100">
                    <span class="text-[#D97706] text-sm uppercase tracking-[0.2em] mb-3 font-semibold">Pilihan Kami</span>
                    <h2 class="text-4xl md:text-5xl font-serif font-bold text-cafe-main mb-6 transition-colors duration-300">Menu Andalan</h2>
                    <div class="w-24 h-1 bg-[#D97706] rounded-full opacity-50"></div>
                </div>

                <!-- Product Grid -->
                <div class="grid grid-cols-1 md:grid-cols-3 gap-10">
                    <!-- Item 1: Nusantara -->
                    <div class="reveal-element delay-200 group block rounded-2xl overflow-hidden bg-cafe-surface border border-cafe-border hover:border-[#D97706]/50 transition-all duration-500 hover:-translate-y-3 relative shadow-sm hover:shadow-xl">
                        <div class="absolute top-4 left-4 z-20 bg-[#D97706] text-white text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-sm shadow-lg">
                            Signature
                        </div>
                        ...
                        <div class="p-8 flex flex-col justify-between h-full bg-cafe-surface transition-colors duration-300">
                            <div>
                                <div class="flex justify-between items-start mb-2">
                                    <h3 class="text-2xl font-serif text-cafe-main group-hover:text-[#D97706] transition-colors">Nasi Goreng Retro</h3>
                                </div>
                            ...
                            <div class="flex items-center justify-between border-t border-cafe-border pt-4 mt-auto">
                                <span class="text-lg text-[#D97706] font-semibold tracking-wide">Rp 45.000</span>
                                <Link v-if="canLogin" :href="route('login')" class="flex items-center gap-2 text-sm uppercase tracking-widest text-cafe-main hover:text-[#D97706] transition-colors group/btn">
                                    Tambah
                                    <span class="w-8 h-8 rounded-full border border-cafe-border flex items-center justify-center group-hover/btn:bg-[#D97706] group-hover/btn:border-[#D97706] group-hover/btn:text-white transition-all">
                    ...
                    <!-- Item 2: Coffee -->
                    <div class="reveal-element delay-300 group block rounded-2xl overflow-hidden bg-cafe-surface border border-cafe-border hover:border-[#D97706]/50 transition-all duration-500 hover:-translate-y-3 relative shadow-sm hover:shadow-xl">
                        ...
                        <div class="p-8 flex flex-col justify-between h-full bg-cafe-surface transition-colors duration-300">
                            <div>
                                <div class="flex justify-between items-start mb-2">
                                    <h3 class="text-2xl font-serif text-cafe-main group-hover:text-[#D97706] transition-colors">Kopi Susu Senja</h3>
                                </div>
                            ...
                            <div class="flex items-center justify-between border-t border-cafe-border pt-4 mt-auto">
                                <span class="text-lg text-[#D97706] font-semibold tracking-wide">Rp 28.000</span>
                                <Link v-if="canLogin" :href="route('login')" class="flex items-center gap-2 text-sm uppercase tracking-widest text-cafe-main hover:text-[#D97706] transition-colors group/btn">
                                    Tambah
                                    <span class="w-8 h-8 rounded-full border border-cafe-border flex items-center justify-center group-hover/btn:bg-[#D97706] group-hover/btn:border-[#D97706] group-hover/btn:text-white transition-all">
                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 5v14M5 12h14"/></svg>
                    ...
                    <!-- Item 3: Dessert -->
                    <div class="reveal-element delay-400 group block rounded-2xl overflow-hidden bg-cafe-surface border border-cafe-border hover:border-[#D97706]/50 transition-all duration-500 hover:-translate-y-3 relative shadow-sm hover:shadow-xl">
                        <div class="aspect-[4/3] overflow-hidden relative">
                            <div class="absolute inset-0 bg-black/10 group-hover:bg-transparent transition-colors z-10 duration-500"></div>
                        ...
                        <div class="p-8 flex flex-col justify-between h-full bg-cafe-surface transition-colors duration-300">
                            <div>
                                <div class="flex justify-between items-start mb-2">
                                    <h3 class="text-2xl font-serif text-cafe-main group-hover:text-[#D97706] transition-colors">Classic Choco Slice</h3>
                                </div>
                            ...
                            <div class="flex items-center justify-between border-t border-cafe-border pt-4 mt-auto">
                                <span class="text-lg text-[#D97706] font-semibold tracking-wide">Rp 35.000</span>
                                <Link v-if="canLogin" :href="route('login')" class="flex items-center gap-2 text-sm uppercase tracking-widest text-cafe-main hover:text-[#D97706] transition-colors group/btn">
                                    Tambah
                                    <span class="w-8 h-8 rounded-full border border-cafe-border flex items-center justify-center group-hover/btn:bg-[#D97706] group-hover/btn:border-[#D97706] group-hover/btn:text-white transition-all">
                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 5v14M5 12h14"/></svg>
        ...
        <section id="cara-pesan" class="py-32 bg-cafe-surface border-y border-cafe-border relative transition-colors duration-300 scroll-mt-20">
            <div class="max-w-7xl mx-auto px-6 relative z-10">
                <div class="text-center mb-20 reveal-element delay-100">
                    <span class="text-[#D97706] text-sm uppercase tracking-[0.2em] mb-3 inline-block font-semibold">Simple & Cepat</span>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-12 relative">
                    ...
                    <div class="hidden md:block absolute top-12 left-[15%] right-[15%] h-[1px] border-t border-dashed border-[#D97706]/30 reveal-element delay-200"></div>

                    <!-- Step 1 -->
                    <div class="relative text-center group reveal-element delay-200">
                        <div class="w-24 h-24 mx-auto bg-cafe-base border border-cafe-border group-hover:border-[#D97706] rounded-full flex items-center justify-center mb-6 relative z-10 transition-colors duration-500 shadow-sm group-hover:shadow-lg">
                            <span class="absolute top-0 right-0 w-8 h-8 bg-[#D97706] text-white font-bold flex items-center justify-center rounded-full transform translate-x-2 -translate-y-2">1</span>
                            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" class="text-[#D97706] group-hover:scale-110 transition-transform duration-500" stroke="currentColor" stroke-width="1.5"><path d="M4 19V6a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v13M4 19h16M4 19l2-2h12l2 2m-8-10v5m0 0l-2-2m2 2l2-2"/></svg>
                        </div>
                    ...
                    <div class="relative text-center group mt-10 md:mt-0 reveal-element delay-300">
                        <div class="w-24 h-24 mx-auto bg-cafe-base border border-cafe-border group-hover:border-[#D97706] rounded-full flex items-center justify-center mb-6 relative z-10 transition-colors duration-500 shadow-sm group-hover:shadow-lg">
                            <span class="absolute top-0 right-0 w-8 h-8 bg-[#D97706] text-white font-bold flex items-center justify-center rounded-full transform translate-x-2 -translate-y-2">2</span>
                            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" class="text-[#D97706] group-hover:scale-110 transition-transform duration-500" stroke="currentColor" stroke-width="1.5"><rect x="3" y="5" width="18" height="14" rx="2"/><line x1="3" y1="10" x2="21" y2="10"/><line x1="7" y1="15" x2="7.01" y2="15"/><line x1="11" y1="15" x2="13" y2="15"/></svg>
                        </div>
                    ...
                    <div class="relative text-center group mt-10 md:mt-0 reveal-element delay-400">
                        <div class="w-24 h-24 mx-auto bg-cafe-base border border-cafe-border group-hover:border-[#D97706] rounded-full flex items-center justify-center mb-6 relative z-10 transition-colors duration-500 shadow-sm group-hover:shadow-lg">
                            <span class="absolute top-0 right-0 w-8 h-8 bg-[#D97706] text-white font-bold flex items-center justify-center rounded-full transform translate-x-2 -translate-y-2">3</span>
                            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" class="text-[#D97706] group-hover:scale-110 transition-transform duration-500" stroke="currentColor" stroke-width="1.5"><path d="M5 12l5 5L20 7"/></svg>
                        </div>
        ...
        <section id="about" class="py-32 relative bg-cafe-base transition-colors duration-300 scroll-mt-20">
            <div class="relative z-10 max-w-5xl mx-auto px-6 text-center reveal-element">
                <svg class="mx-auto mb-8 w-12 h-12 text-[#D97706]/50" viewBox="0 0 24 24" fill="currentColor">
                ...
                <h2 class="text-3xl md:text-5xl font-serif font-bold mb-8 leading-tight text-cafe-main transition-colors duration-300">Membawa Kenangan Klasik <br/>dalam Sajian <span class="text-[#D97706]">Masa Kini</span></h2>
                ...
                <div class="mt-12 w-16 h-[1px] bg-[#D97706]/50 mx-auto"></div>
            </div>
        </section>
        ...
        <footer class="bg-cafe-surface py-16 border-t border-cafe-border relative z-10 text-center transition-colors duration-300">
            <div class="max-w-7xl mx-auto px-6 reveal-element delay-100">
                <!-- Branding -->
                <div class="mb-8">
                    <h2 class="text-3xl font-serif font-bold text-cafe-main mb-2 logo-anim transition-colors duration-300">Denjavas<span class="text-[#D97706]">.</span></h2>
                    <p class="text-[#D97706]/70 text-xs tracking-[0.3em] uppercase font-semibold">Retro Café</p>
                </div>
                ...
                <div class="flex justify-center gap-6">
                    <a href="#" class="text-cafe-muted hover:text-[#D97706] transition-colors">
